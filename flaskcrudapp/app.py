...
app.config['MYSQL_USER'] = 'bf23508f07c6b1'
app.config['MYSQL_PASSWORD'] = '66abd0d6'
app.config['MYSQL_DB'] = 'heroku_fa1b9258fb47ed9'
mysql = MySQL(app)

# Settings
app.secret_key = 'mysecretkeyz1' # Tell how our session will be configured (this starts the sessions)
# Defining our Routes
@app.route('/')
def Index():
...
def get_contact(id):
    cur = mysql.connection.cursor()
    cur.execute('SELECT * FROM contacts where id = %s', [id]) # It's interesting that with () it won't work
    data = cur.fetchall()
